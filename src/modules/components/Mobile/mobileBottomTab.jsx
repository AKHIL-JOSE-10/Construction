...
const MobileBottomTab = () => {
    ...
    const getTabValue = (pathname) => {
        ...
        if (pathname.startsWith('/mobile-premium')) return 1;
        if (pathname.startsWith('/mobile-messages')) return 2;
        if (pathname.startsWith('/mobile-bookings')) return 3;
        if (pathname.startsWith('/mobile-profile')) return 4;
        return 0;
    };
    ...
    const handleChange = (event, newValue) => {
        ...
        else if (newValue === 1) navigate('/mobile-premium');
        else if (newValue === 2) navigate('/mobile-messages');
        else if (newValue === 3) navigate('/mobile-bookings');
        else if (newValue === 4) navigate('/mobile-profile');
    };
        ...
        <>
            ...
            <Box sx={{ mt: 13, position: 'fixed', overflow: 'hidden', bottom: 0, left: 0, right: 0, width: '100%', bgcolor: 'white', zIndex: 10, borderTop: '1px solid #ccc' }}>
                ...
                >
                    ...
                    <Tab icon={<WorkspacePremiumIcon sx={{ width: 24, height: 24 }} />} label="Premium" sx={{ fontSize: '0.6rem', minWidth: 0, px: 0.8, mx: 0.4, py: 0.2, my: 0.2, textTransform: 'none', minHeight: 45, height: 45 }} />
