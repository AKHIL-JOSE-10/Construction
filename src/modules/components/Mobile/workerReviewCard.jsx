import { Grid, Avatar, Box, Rating, Stack, Typography, Paper, Select, MenuItem, } from '@mui/material';


const WorkerReviewCard = ({name,rate,date}) => {
    return (
                ...
                <Box sx={{ display: 'flex', flexDirection: 'row' }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <Stack direction="row" spacing={1} alignItems="center">
                            <Typography sx={{ fontWeight: 'bold', fontSize: '18px' }}>{rate}</Typography>
                            <Rating size="small" name="half-rating-read" defaultValue={rate} precision={0.5} readOnly />
                        </Stack>
                    </Box>
                </Box>
                <Typography sx={{ color: 'grey', fontSize: '15px', pr: 1 }}>{date}</Typography>
            </Box>
            <Box sx={{ maxWidth: 600, mx: 'auto' }}>
                <Typography sx={{ mt: 3, fontSize: 16, lineHeight: 1.4, textAlign: 'justify' }}>
